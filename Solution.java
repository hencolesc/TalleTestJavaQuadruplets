...
class Solution {
    ...
    public static List<Integer[]> fourNumberSum(Integer[] array, int targetSum) {
        ...
        // return array of arrays of positions
        quadruplets = allQuadruplets(array);

        quadruplets.forEach { it ->
            long sum = 0;
    ...
    public static List<Integer[]> allQuadruplets(Integer[] array) {
        ArrayList<Integer[]> quadruplets = new ArrayList<Integer[]>();

        array.forEach { it ->
            long indexOfIt = array.indexOf(it);

            if(array.size - indexOfIt >= 0) {
                quadruplets.add(new {array[indexOfIt], array[indexOfIt + 1], array[indexOfIt + 2], array[indexOfIt + 3]});
            }
        }
